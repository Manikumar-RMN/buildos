  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className="mx-auto max-w-7xl px-6 py-8"><div className="mb-6"><p className="text-sm font-medium text-slate-500">Site activity</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Daily Progress</h1><p className="mt-2 text-sm text-slate-500">Capture a quick daily site update and turn field activity into project visibility.</p></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[380px_1fr]"><form onSubmit={saveProgress} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Today’s update</h2><div className="mt-4 space-y-3">
        ...
        <label className="block text-sm font-medium text-slate-700">Weather<input value={form.weather} onChange={e => setForm({ ...form, weather: e.target.value })} placeholder="e.g. Sunny" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 placeholder:text-slate-400" /></label>
        <label className="block text-sm font-medium text-slate-700">Work completed<textarea rows={3} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="What was completed today?" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 placeholder:text-slate-400" /></label>
        <label className="block text-sm font-medium text-slate-700">Issues / blockers<textarea rows={2} value={form.issues} onChange={e => setForm({ ...form, issues: e.target.value })} placeholder="Anything the owner or PM should know?" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-900 placeholder:text-slate-400" /></label>
        <label className="block text-sm font-medium text-slate-700">Site photos<input id="progress-photos" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={e => setPhotos(Array.from(e.target.files ?? []))} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-sm" /></label>
        {photos.length > 0 && <p className="text-xs text-slate-500">{photos.length} photo{photos.length === 1 ? '' : 's'} selected • Maximum 10 MB each</p>}
        <button disabled={saving} className="w-full rounded-lg bg-blue-600 px-4 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50">{saving ? 'Saving update...' : 'Save daily update'}</button></div></form>
        <section className="space-y-3">{records.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">No daily updates yet.</div> : records.map(r => <article key={r.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">{r.projects?.name || 'Project'}</h2><p className="mt-1 text-sm text-slate-500">{r.project_sites?.name || 'No site'} • {r.progress_date}</p></div>{r.progress_percent !== null && <span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700">{r.progress_percent}%</span>}</div>{r.notes && <p className="mt-3 text-sm text-slate-700">{r.notes}</p>}{r.issues && <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">Issue: {r.issues}</p>}{r.weather && <p className="mt-2 text-xs text-slate-500">Weather: {r.weather}</p>}{r.daily_progress_photos && r.daily_progress_photos.length > 0 && <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">{r.daily_progress_photos.map(photo => photo.signedUrl ? <a key={photo.id} href={photo.signedUrl} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-lg border border-slate-200 bg-slate-100"><img src={photo.signedUrl} alt="Site progress" className="h-28 w-full object-cover transition group-hover:scale-105" /></a> : null)}</div>}</article>)}</section>
      </div>
    </div></main>
}
